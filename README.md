...
|----------|--------------------------------|----------------|-------------|
| `core`   | `io.github.edadma::sdl3`       | `SDL3`         | Window (incl. resize events and display usable bounds), float render API (incl. `RenderGeometry` fills), textures (incl. YUV video frames with GPU colour conversion), surfaces, events (incl. OS file drop), keyboard/mouse, system cursors, clipboard, native file dialogs, hints, render targets, PCM audio playback |
...
```scala
libraryDependencies += "io.github.edadma" %%% "sdl3"       % "0.2.10"
libraryDependencies += "io.github.edadma" %%% "sdl3_ttf"   % "0.2.10"
libraryDependencies += "io.github.edadma" %%% "sdl3_image" % "0.2.10"
libraryDependencies += "io.github.edadma" %%% "sdl3_mixer" % "0.2.10"
```
